...
from utils.config import (
    ...
    DEFAULT_MARGIN, DEFAULT_P_COEFF
)
...
class PenaltyScoredFusionSearchEngine:
    def __init__(self):
    ...
    def hybrid_search(
        self, 
        query: str, 
        semantic_weight: float = PENALTY_WEIGHTS['semantic'],
        lexical_weight: float = PENALTY_WEIGHTS['lexical'],
        recency_weight: float = PENALTY_WEIGHTS['recency'],
        decay_base: float = DEFAULT_DECAY_BASE,
        half_life_days: float = DEFAULT_HALF_LIFE_DAYS,
        top_k: int = DEFAULT_TOP_K,
        margin: float = DEFAULT_MARGIN,
        p_coeff: float = DEFAULT_P_COEFF
    ) -> List[Dict]:
        """
        Combine semantic, lexical, and recency search results using scored fusion with a penalty term.
        """
        if not self.chunk_data:
            return []
        ...
        logger.info(f"Hybrid search with penalty scored fusion: '{query[:50]}...'")
        ...
        # Apply scored fusion with penalty
        combined_scores = {}
        for chunk_id in semantic_scores:
            semantic_score = norm_semantic[chunk_id]
            lexical_score = norm_lexical[chunk_id]
            recency_score_raw = norm_recency[chunk_id]
            
            # Calculate relevance score
            relevance_score = (semantic_weight * semantic_score) + (lexical_weight * lexical_score)
            
            # Calculate weighted recency score
            recency_score = recency_weight * recency_score_raw
            
            # Calculate penalty
            diff = recency_score - relevance_score - margin
            # Apply ReLU function for penalty
            penalty = p_coeff * np.maximum(0, diff)
            
            # Final score
            final_score = relevance_score + recency_score - penalty
        ...
        for chunk_id, final_score in top_results:
            ...
            results.append({
                ...
                'recency_score': recency_score_raw,
...
if __name__ == "__main__":
    search_engine = PenaltyScoredFusionSearchEngine()
    ...
    if search_engine.chunk_data:
        ...
        print(f"\nHybrid Search Results for Penalty Scored Fusion: '{query}'")
        print("=" * 80)

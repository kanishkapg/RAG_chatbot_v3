...
class MultiplicativeGatingSearchEngine:
    def __init__(self):
        """Initialize the hybrid search engine with embedding model and TF-IDF vectorizer."""
        logger.info("Initializing Multiplicative Gating Search Engine...")
        ...
        logger.info("Multiplicative Gating Search Engine initialized successfully.")
    ...
    def hybrid_search(
        self, 
        query: str, 
        semantic_weight: float = MULTIPLICATIVE_WEIGHTS['semantic'],
        lexical_weight: float = MULTIPLICATIVE_WEIGHTS['lexical'],
        recency_weight: float = MULTIPLICATIVE_WEIGHTS['recency'],
        decay_base: float = DEFAULT_DECAY_BASE,
        half_life_days: float = DEFAULT_HALF_LIFE_DAYS,
...
if __name__ == "__main__":
    search_engine = MultiplicativeGatingSearchEngine()
    ...
    if search_engine.chunk_data:
        ...
        print(f"\nHybrid Search Results for Multiplicative Gating: '{query}'")
        print("=" * 80)
